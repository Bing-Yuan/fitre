    ...
    def step(self, closure=None):
        ...
        for i, m in enumerate(self.modules):
            ...
            if self.steps % self.Tf == 0:
                ...
                self.aa_inv[m] = torch.inverse(self.m_aa[m] + la * torch.eye(self.m_aa[m].shape[0], device=p.device))
                self.gg_inv[m] = torch.inverse(self.m_gg[m] + la * torch.eye(self.m_gg[m].shape[0], device=p.device))
